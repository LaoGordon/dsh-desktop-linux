...
function launchForPort (runtime, port) {
  // Extra CLI args injected into the spawned `dsh web`, e.g. to load plugins
  // declared via `--patch` without recompiling. These are opt-in environment
  // knobs: `DSH_DESKTOP_EXTRA_ARGS` (shell-split arbitrary args) and
  // `DSH_DESKTOP_PATCH` (a single --patch path, kept separate for convenience).
  let extra = []
  const patchEnv = process.env.DSH_DESKTOP_PATCH
  if (patchEnv) extra.push('--patch', patchEnv)
  const extraEnv = process.env.DSH_DESKTOP_EXTRA_ARGS
  if (extraEnv) extra = extra.concat(extraEnv.match(/(?:[^\s"]+|"[^"]*")+/g) || [])
  return {
    ...runtime,
    args: [
      ...runtime.prefixArgs,
